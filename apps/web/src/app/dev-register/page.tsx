...
export default function DevRegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    // Logika validasi
    if (!instanceName || !npwp || !address || !website || !sector || !contactName || !position || !email || !phone || !usagePurpose || !expectedVolume) {
      setError('Mohon lengkapi semua data yang diperlukan.');
      return;
    }

    const newId = `inst-${Math.random().toString(36).substr(2, 9)}`;
    const newApiKey = `api-key-${Math.random().toString(36).substr(2, 12)}`;

    setRegisteredDetails({ id: newId, apiKey: newApiKey });
      ...
      <div className={styles.authCard}>
        ...
        <form onSubmit={handleRegister} className={styles.authForm}>
          ...
          <div className={styles.formGrid}>
            {/* Kolom 1: Informasi Legal dan Bisnis */}
            ...
            {/* Kolom 2: Informasi Kontak Penanggung Jawab */}
            <div className={styles.formSection}>
            ...
            {/* Kolom 3: Informasi Teknis dan Tujuan */}
            <div className={styles.formSection}>
              ...
              <div className={styles.inputGroup}>
                ...
                <select id="integrationMethod" value={integrationMethod} onChange={(e) => setIntegrationMethod(e.target.value)} className={styles.authInput}>
                  <option value="">Pilih metode integrasi</option>
                  <option value="API">API (Extremely Recomended)</option>
